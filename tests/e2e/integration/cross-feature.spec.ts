...
test.describe('Cross-Feature Integration', () => {
  ...
  test.describe('Notes & Presets Integration', () => {
    test('should use custom types in filter presets', async ({ page }) => {
      ...
      await typeDialog.locator('[data-testid="type-color"]').click();
      await page.locator('[aria-label="Select color #FF6B6B"]').click();
    ...
    test('should use custom priorities in notes creation', async ({ page }) => {
      ...
      await priorityDialog.locator('[data-testid="priority-color"]').click();
      // Use a color that exists in the picker - #9b59b6 is not in the default list, using #8B5CF6 (Purple) instead
      await page.locator('[aria-label="Select color #8B5CF6"]').click();
  ...
  test.describe('Theme and UI Consistency', () => {
    test('should maintain module colors across all interfaces', async ({ page }) => {
      ...
      for (const mod of modules) {
        await helpers.navigateToModule(mod.name);

        // Check main module header
        const header = page.locator(`h1:has-text("${mod.name.replace('-', ' ')}")`);
        await expect(header.locator(`.${mod.colorClass}`)).toBeVisible();
        ...
        await expect(dialogHeader).toHaveClass(new RegExp(mod.theme));
        ...
        await expect(emailHeader).toHaveClass(new RegExp(mod.theme));
