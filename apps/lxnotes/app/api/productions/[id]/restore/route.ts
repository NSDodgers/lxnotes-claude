...
/**
 * POST /api/productions/[id]/restore
 * Restore a soft-deleted production from trash - production admin only
 */
export async function POST(
  ...
  try {
    ...
    if (!isAdmin) {
      return NextResponse.json({ error: 'Forbidden - only production admins can restore' }, { status: 403 })
    }
