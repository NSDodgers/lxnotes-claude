-- Remove redundant title column from notes table.
-- The app only has a single "Note Content" input field that saves the same
-- value to both title and description. The title column was never used
-- independently — every code path either shows description or falls back
-- to title. Consolidate to description only.

-- 1. Backfill: copy title into description for any rows where description is null
UPDATE notes SET description = title WHERE description IS NULL;

-- 2. Make description NOT NULL now that it's the sole content field
ALTER TABLE notes ALTER COLUMN description SET NOT NULL;
ALTER TABLE notes ALTER COLUMN description SET DEFAULT '';

-- 3. Drop the redundant title column
ALTER TABLE notes DROP COLUMN title;

-- 4. Recreate import_production_snapshot without the title column.
--    Old snapshots may still contain a "title" key in the JSON — it is
--    simply ignored because the INSERT no longer lists it.
...
BEGIN
  ...
  IF p_mode = 'restore' THEN
    ...
    IF jsonb_array_length(COALESCE(p_snapshot->'scriptPages', '[]'::JSONB)) > 0 THEN
      INSERT INTO script_pages (id, production_id, page_number, first_cue_number, act_name, act_first_cue_number, created_at, updated_at)
      SELECT
        (elem->>'id')::UUID,
        v_target_id,
        elem->>'page_number',
    ...
    -- Note: old snapshots may contain a "title" key which is simply ignored.
    -- description is populated from the snapshot's description, falling back
    -- to title for backward compatibility with pre-migration snapshots.
    IF jsonb_array_length(COALESCE(p_snapshot->'notes', '[]'::JSONB)) > 0 THEN
      ...
      SELECT
        ...
        v_target_id,
        elem->>'module_type',
        COALESCE(elem->>'description', elem->>'title', ''),
        elem->>'type',
  ...
  ELSIF p_mode = 'clone' THEN
    ...
    -- Notes: description falls back to title for old snapshots
    IF jsonb_array_length(COALESCE(p_snapshot->'notes', '[]'::JSONB)) > 0 THEN
      INSERT INTO notes (
        id, production_id, module_type, description, type, priority, status,
        created_by, assigned_to, completed_by, completed_at, due_date,
