...
export const useFixtureStore = create<FixtureState>()(
  persist(
    (set, get) => ({
      ...
      // Sync fixtures from external source (e.g. Supabase/Realtime)
      syncFixtures: (productionId: string, externalFixtures: FixtureInfo[]) => {
        const state = get()

        // Replace fixtures for this production, keep others
        const otherFixtures = state.fixtures.filter(f => f.productionId !== productionId)
        const updatedFixtures = [...otherFixtures, ...externalFixtures]
